...
static void run_test(bench_data* data, size_t count, int includes_g, int n_points) {
    char str[32];
    ...
    /* Run the benchmark. */
    sprintf(str, includes_g ? "ecmult_%ig" : "ecmult_%i", (int)n_points);
    run_benchmark(str, bench_ecmult, bench_ecmult_setup, bench_ecmult_teardown, data, 100, n_points * (1 + ITERS / count));
}

int pippenger_split(const secp256k1_ecmult_context *ctx, secp256k1_scalar *scalars, secp256k1_gej *points, secp256k1_scalar *seckeys, const secp256k1_scalar *scalar, const secp256k1_gej *point, secp256k1_scalar *seckey, unsigned int n) {
        secp256k1_gej acc1;
        secp256k1_gej acc2;
        unsigned int i;
        ...
        for(i = 0; i < n; i++) {
            int n_bits = 256/n;
            secp256k1_scalar foo;
            secp256k1_scalar precomp;
            unsigned char bytes1[32] = { 0 };
            unsigned char bytes2[32] = { 0 };
            int overflow;
            secp256k1_scalar_get_b32(bytes1, scalar);
            memcpy(bytes2 + 32 - n_bits/8, bytes1 + 32 - (i+1)*n_bits/8, n_bits/8);
            secp256k1_scalar_set_b32(&foo, bytes2, &overflow);
            secp256k1_scalar_get_b32(bytes1, &foo);
            if (overflow) {
                return 0;
            }
            scalars[i] = foo;
            secp256k1_scalar_clear(&precomp);
            secp256k1_scalar_cadd_bit(&precomp, i*n_bits, 1);
            secp256k1_ecmult(ctx, &points[i], point, &precomp, NULL);
            secp256k1_scalar_mul(&seckeys[i], &precomp, seckey);
        }
        secp256k1_gej_set_infinity(&acc1);
        secp256k1_gej_set_infinity(&acc2);
        for(i = 0; i < n; i++) {
            secp256k1_gej r;
            secp256k1_gej_set_infinity(&r);
            secp256k1_ecmult(ctx, &r, &points[i], &scalars[i], NULL);
            secp256k1_gej_add_var(&acc1, &r, &acc1, NULL);
        }
        secp256k1_gej_neg(&acc1, &acc1);
        secp256k1_ecmult(ctx, &acc2, point, scalar, NULL);
        secp256k1_gej_add_var(&acc1, &acc2, &acc1, NULL);
        assert(secp256k1_gej_is_infinity(&acc1));
        return 1;
}
...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < POINTS/PRECOMPS; ++i) {
        ...
        if (i) {
            ...
            secp256k1_scalar_add(&seckeys[i], &seckeys[i - 1], &seckeys[i - 1]);
        }
