...
int secp256k1_schnorrsig_sign(const secp256k1_context* ctx, secp256k1_schnorrsig *sig, const unsigned char *msg32, const unsigned char *seckey, secp256k1_nonce_function noncefp, void *ndata) {
    ...
    /* Because we are signing for a schnorrsig pubkey, the secret key is
     * negated before signing if the Y coordinate is not a quadratic residue */
 ...
 * Computes R = sG - eP. */
static int secp256k1_schnorrsig_real_verify(const secp256k1_context* ctx, secp256k1_gej *rj, const secp256k1_scalar *s, const secp256k1_scalar *e, const secp256k1_positive_pubkey *pk) {
...
int secp256k1_schnorrsig_verify(const secp256k1_context* ctx, const secp256k1_schnorrsig *sig, const unsigned char *msg32, const secp256k1_positive_pubkey *pk) {
    secp256k1_scalar s;
    secp256k1_scalar e;
    secp256k1_gej rj;
    secp256k1_fe rx;
    ...
    secp256k1_schnorrsig_sha256_tagged(&sha);
    secp256k1_sha256_write(&sha, &sig->data[0], 32);
    secp256k1_positive_pubkey_serialize(ctx, buf, pk);
    secp256k1_sha256_write(&sha, buf, sizeof(buf));
    secp256k1_sha256_write(&sha, msg32, 32);
    secp256k1_sha256_finalize(&sha, buf);
    secp256k1_scalar_set_b32(&e, buf, NULL);
...
static int secp256k1_schnorrsig_verify_batch_ecmult_callback(secp256k1_scalar *sc, secp256k1_ge *pt, size_t idx, void *data) {
    ...
    /* eP */
        ...
        secp256k1_positive_pubkey_serialize(ecmult_context->ctx, buf, ecmult_context->pk[idx / 2]);
        secp256k1_sha256_write(&sha, buf, sizeof(buf));
 ...
 */
static int secp256k1_schnorrsig_verify_batch_init_randomizer(const secp256k1_context *ctx, secp256k1_schnorrsig_verify_ecmult_context *ecmult_context, secp256k1_sha256 *sha, const secp256k1_schnorrsig *const *sig, const unsigned char *const *msg32, const secp256k1_positive_pubkey *const *pk, size_t n_sigs) {
    size_t i;
    ...
    for (i = 0; i < n_sigs; i++) {
         ...
         * positive_pubkey serialization and a user would wrongly memcpy
         * normal libsecp pubkeys into positive_pubkeys then the randomizer
         * would be the same for two different pubkeys with catastrophic
         * results */
...
int secp256k1_schnorrsig_verify_batch(const secp256k1_context *ctx, secp256k1_scratch *scratch, const secp256k1_schnorrsig *const *sig, const unsigned char *const *msg32, const secp256k1_positive_pubkey *const *pk, size_t n_sigs) {
