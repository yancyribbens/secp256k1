...
void bench_schnorrsig_verify(void* arg) {
    ...
    for (i = 0; i < 1000; i++) {
        ...
        CHECK(secp256k1_positive_pubkey_parse(data->ctx, &pk, data->pk[i]) == 1);
...
void bench_schnorrsig_verify_n(void* arg) {
    bench_schnorrsig_data *data = (bench_schnorrsig_data *)arg;
    size_t i, j;
    const secp256k1_positive_pubkey **pk = (const secp256k1_positive_pubkey **)malloc(data->n * sizeof(*pk));

    CHECK(pk != NULL);
    for (j = 0; j < MAX_SIGS/data->n; j++) {
        for (i = 0; i < data->n; i++) {
            secp256k1_positive_pubkey *pk_nonconst = (secp256k1_positive_pubkey *)malloc(sizeof(*pk_nonconst));
            CHECK(secp256k1_positive_pubkey_parse(data->ctx, pk_nonconst, data->pk[i]) == 1);
            pk[i] = pk_nonconst;
        }
...
int main(void) {
    ...
    for (i = 0; i < MAX_SIGS; i++) {
        ...
        CHECK(secp256k1_positive_pubkey_create(data.ctx, &pk, sk));
        CHECK(secp256k1_positive_pubkey_serialize(data.ctx, pk_char, &pk) == 1);
        CHECK(secp256k1_schnorrsig_sign(data.ctx, sig, msg, sk, NULL, NULL));
    }
