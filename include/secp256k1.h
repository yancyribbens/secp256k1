...
/** Converts a secp256k1_pubkey into a secp256k1_positive_public_key. This function
 *  optionally outputs a sign bit that can be used to convert the
 *  secp256k1_positive_public_key back into a secp256k1_public_key.
 *  The sign bit is 1 if the input pubkey is positive (has a Y coordinate that is
 *  a quadratic residue), otherwise it is 0.
 *
 *  Returns: 1: the public key was successfully converted
 *           0: otherwise
 *  Args:    ctx:             pointer to a context object
...
/** Convert a secp256k1_positive_pubkey into a secp256k1_public_key.  This function
 *  takes the sign bit that was output of secp256k1_positive_pubkey_from_signed.
 *  If the sign bit is not 0 the output pubkey is positive (has a Y coordinate that
 *  is a quadratic residue), otherwise it is . negative.
 ...
 *  input public key.
 *  Returns: 1: the tweak was successfully added to seckey
 *           0: the tweak was out of range or the resulting private key would be
 ...
 *  Returns: 1: tweak times the generator was successfully added to pubkey
 *           0: the tweak was out of range or the resulting public key would be
 ...
 *  Returns: 1: output_pubkey is the result of tweaking the internal_pubkey with
 *              tweak32
 *           0: otherwise
 *  Args:    ctx: pointer to a context object initialized for validation
                  (cannot be NULL)
